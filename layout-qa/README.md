...
| 4 | 2026-07-22 | Gateway exit-port distribution for >2 branches (exe-inspection feedback). rep_2b's 3-way gateway fired two of its three exits 2px apart (y 64/66), so those connectors overprinted (the label stack was fine, the underlying ports were not). Fix: after the fan-out, respace any gateway with ≥3 exits EVENLY around its centre, `GW_EXIT_GAP`=22px apart, preserving the fan's crossing-minimising order; with the ports now spread, each branch label rides just above its OWN connector (≤2-branch gateways keep the stacked-above layout). Guarded by a new invariant (exit ports ≥8px apart) plus the label-proximity/overprint ones | **0 on all 12 diagrams**; ratchet exits 0 after baseline update | Deliberate, user-requested trade on `rep_2b_motor_donante` only: distributing the 3 exits bends one same-row branch and adds one gutter crossing — `crossings.total` 2→3, `straightness.straightPct` 100→87.5 (2 sameRowBends), `meanEdgeLength` 208.7→210.1. Accepted: legible, non-overlapping exits were the explicit goal; the bend is a few px near the gateway | `d98d0e9` |
| 5 | 2026-07-22 | Gateway fan-out rework — label-with-connector + straight branches (exe-inspection feedback, sketches 9/10/12). Two remaining issues: (a) ≤2-branch gateways STACKED both labels above the top exit (detached from their branches); (b) `gw_decide`'s "Reparar con alternativo" → the "Sigue con alternativo" end event drew a Z-step because the event stayed at its slot centre while the branch's exit was spread. Fix: unify to a single per-exit-label rule for every fan size; anchor the straight-row branch on its TARGET's entry port (exit == entry → truly straight, killing even a 2px step); spread up/down branches `GW_EXIT_GAP` apart, recentring only on gateway-face overflow; and NUDGE a movable leaf-event target onto the exit so the branch stays straight (the event's height, which the layouter previously didn't adjust). Guarded by new invariants: every label just above ITS OWN exit; a leaf-event branch near the gateway row renders as 2 waypoints (no Z-step) | **0 on all 12 diagrams**; ratchet exits 0 | All IMPROVEMENTS, no regressions: `rep_2b_motor_donante` recovers the round-4 straightness (`straightPct` 87.5→100, `sameRowBends`/`dodges` 2→0), `crossings.total` 3→2, `meanEdgeLength` 210→207; `rep_2_diagnostico`/`rep_4_reparacion` also lower `meanEdgeLength` | `2a11a92` |
| 6 | 2026-07-23 | Connector docking on the real shape outline + 1px branch labels (exe-inspection feedback). (a) Waypoints touching a gateway docked on the bounding-box corner while ports were spread vertically — at a y off the centre the diamond outline is inset, so connectors floated up to 22px off the shape and read as DISCONNECTED in the modeler (moving the gateway made bpmn-js re-dock them: the tell). New `dockX`/`dockY` anchor every touching waypoint on the actual outline (diamond linear inset, event-circle chord; rects unchanged), all four plan kinds. (b) Branch labels sat 6px above their connector ("muy separadas"): label box bottom now exactly 1px above its own exit line, or box top 1px below when the lane top leaves no room. Guarded by new DI invariants: waypoint on the diamond perimeter (\|dx\|/hw + \|dy\|/hh = 1, ≤2px); 1px label hug | **0 on all 12 diagrams**; ratchet exits 0 after baseline update | Cohesion `totalEdgeLength` grows on gateway-heavy reps because docked waypoints now reach the outline (the drawn gap closed — longer measured, visually attached); part of the delta is live qa-workspace drift since round 5 (e.g. `rep_4` gained 3 flows). `flujo_reparaciones_novotec` `bboxArea` 848k→636k (workspace drift, improvement) | `603d191` |
