...
| 3 | 2026-07-22 | Branch-label placement fix (exe-inspection feedback): centre each gateway's branch-label stack on the gateway instead of the tall row's top edge (labels were floating ~40px above the gateway they annotate), and widen the inter-label gap 4px→10px (rep_2b's 3-way gateway had two labels 4px apart that read as overlapping). Stack bottom is clamped above the gateway's lower edge so a 3-branch stack clears the gateway's own name label. Guarded by new DI-level invariants in `layoutElkLanes.test.ts` | **0 on all 12 diagrams**; ratchet exits 0, no soft regressions (labels moved within existing bounds — no metric shifted past its stored value) | Label geometry only; no measured soft-metric change. Visual: branch labels now hug their gateway across every laned diagram | `_pendiente_` |
