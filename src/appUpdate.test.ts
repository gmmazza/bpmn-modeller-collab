...
describe("evaluateUpdate", () => {
  it("reports an available update when the feed is newer, passing the asset URL through", () => {
    expect(
      evaluateUpdate("0.1.0", { version: "0.2.0", url: "http://x/release", asset: "http://x/app.zip" }),
    ).toEqual({
      updateAvailable: true,
      latest: "0.2.0",
      url: "http://x/release",
      asset: "http://x/app.zip",
    });
  });
  it("defaults asset to empty string when the feed omits it", () => {
    expect(evaluateUpdate("0.1.0", { version: "0.2.0", url: "u" }).asset).toBe("");
  });
